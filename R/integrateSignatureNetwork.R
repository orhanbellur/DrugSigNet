...
#' Integrate Signature Results with Raw Network-Based Results
#'
#' @description
#' Integrates signature results with raw network outputs. This variant expects
#' network method results in `network_res$DrugSearching$Raw`, builds processed
#' rank tables for centrality and P1/P2/P3 proximity outputs, harmonizes P1/P2/P3
#' and Diffusion separately with CRank, Dowdall, and RRA, and then integrates the
#' resulting network harmonized table with signature pairwise rankings.
#'
#' @inheritParams integrateSignatureNetwork
#'
#' @return A `DrugSearchingPipeline` S4 object with `type = "integration"`.
#'   The `DrugSearching$Processed` slot contains processed raw network method
#'   tables with `Rank` columns where applicable. The `RankAggregation` slot
#'   contains network proximity/diffusion harmonization outputs plus integrated
#'   signature-network CRank, Dowdall, RRA, and harmonized tables.
#'
#' @export
integrateSignatureNetworkRaw <- function(signature_res,
                                         network_res,
                                         ties_method = "max",
                                         prior = 0.093,
                                         num_bin = 200,
  ...
  run_drug_annotation <- .pipeline_flag(run_drug_annotation, "run_drug_annotation")
  run_visualization <- .pipeline_flag(run_visualization, "run_visualization")
  .pipeline_message("Integration", "Validating raw network payloads and options.", 1, 7)
  ...
  .pipeline_message("Integration", "Extracting signature ranks and raw network components.", 2, 7)
  signature_payload <- .integration_payload(signature_res, "signature_res")
  network_payload <- .integration_payload(network_res, "network_res")
  ...
  network_components <- .integration_raw_network_components(
    network_payload = network_payload,
    ties_method = ties_method,
    prior = prior,
    num_bin = num_bin
  )
  network_harmonized_df <- network_components$Network_Harmonized
  network_harmonized_df <- .integration_normalize_id(network_harmonized_df, "network_raw_harmonized")
  network_harmonized_df <- .integration_expand_network_drugs(network_harmonized_df)

  signature_drugs <- unique(unlist(lapply(signature_method_tables, function(x) x$Drug), use.names = FALSE))
  common_drugs <- intersect(signature_drugs, network_harmonized_df$Drug)
  if (length(common_drugs) == 0) {
    stop("No overlapping drugs between signature and raw network results.", call. = FALSE)
  }

  .pipeline_message("Integration", "Preparing integrated rank aggregation inputs.", 3, 7)
  Signature_Network_CRank_input <- .integration_method_input(
    sig_df = signature_method_tables$CRank,
    net_df = network_harmonized_df,
    common_drugs = common_drugs,
    method = "CRank"
  )
  ...
  Signature_Network_Harmonized <- dplyr::inner_join(
    sig_df,
    network_harmonized_df %>% .integration_expand_network_drugs(),
    by = "Drug"
  ) %>%
    dplyr::full_join(Signature_Network_CRank@result %>% .integration_expand_network_drugs(), by = "Drug") %>%
    dplyr::full_join(
  ...
  rank_aggregation <- c(
    network_components$RankAggregation,
    list(
      Signature_Network_CRank = Signature_Network_CRank,
      Signature_Network_Dowdall = Signature_Network_Dowdall,
      Signature_Network_RRA = Signature_Network_RRA,
  ...
  pipeline_builder(
    processed = network_components$Processed,
...
.integration_rank_values <- function(values, ties_method, decreasing = TRUE) {
  .rank_values(values, ties_method, decreasing = decreasing, na_fill = 0)
}

.integration_processed_raw_method <- function(network_payload,
                                              method,
                                              ties_method,
                                              decreasing = TRUE) {
  df <- .integration_result_df(network_payload$DrugSearching$Raw[[method]])
  if (is.null(df) || !is.data.frame(df)) {
    return(NULL)
  }
  df <- .safe_rename_drug(df)
  ...
  df <- .integration_normalize_id(df, paste0("network_res$DrugSearching$Raw$", method))
  score_col <- if ("Score" %in% names(df)) {
    "Score"
  ...
  df$Rank <- .integration_rank_values(
    df[[score_col]],
    ties_method = ties_method,
...
.integration_processed_rank_input <- function(processed, method) {
  df <- processed[[method]]
  if (is.null(df) || !is.data.frame(df) || !nrow(df) || !"Rank" %in% names(df)) {
    return(NULL)
  }

  df %>%
    dplyr::select(Drug, Rank) %>%
    dplyr::rename(!!method := Rank) %>%
    dplyr::distinct()
}

.integration_run_raw_network_aggregation <- function(input_df,
                                                     prefix,
                                                     ties_method,
                                                     prior,
                                                     num_bin) {
  if (is.null(input_df) || !is.data.frame(input_df) || !nrow(input_df)) {
    return(list(aggregations = list(), harmonized = NULL))
  }
  numeric_cols <- .integration_numeric_rank_cols(input_df)
  if (length(numeric_cols) < 2) {
    return(list(aggregations = list(), harmonized = input_df))
  }
  ...
  agg_crank@result <- agg_crank@result %>%
    .integration_normalize_id(paste0(prefix, "_CRank")) %>%
    dplyr::rename(!!paste0(prefix, "_CRank") := CRank)
  ...
  agg_dowdall@result <- agg_dowdall@result %>%
    .integration_normalize_id(paste0(prefix, "_Dowdall")) %>%
    dplyr::rename(!!paste0(prefix, "_Dowdall") := Dowdall_rank)
  ...
  agg_rra@result <- agg_rra@result %>%
    .integration_normalize_id(paste0(prefix, "_RRA")) %>%
    dplyr::rename(!!paste0(prefix, "_RRA") := RRA_rank)

  harmonized <- Reduce(
    function(x, y) dplyr::full_join(x, y, by = "Drug"),
    list(
      agg_crank@result,
      agg_dowdall@result %>% dplyr::select(Drug, dplyr::all_of(paste0(prefix, "_Dowdall"))),
      agg_rra@result %>% dplyr::select(Drug, dplyr::all_of(paste0(prefix, "_RRA")))
    )
  ) %>%
    dplyr::distinct(Drug, .keep_all = TRUE)

  list(
    aggregations = stats::setNames(
      list(agg_crank, agg_dowdall, agg_rra),
      c(paste0(prefix, "_CRank"), paste0(prefix, "_Dowdall"), paste0(prefix, "_RRA"))
    ),
    harmonized = harmonized
  )
}

.integration_raw_network_components <- function(network_payload,
                                                ties_method,
                                                prior,
                                                num_bin) {
  processed <- list(
    TrustRank = .integration_processed_raw_method(network_payload, "TrustRank", ties_method, decreasing = TRUE),
    Harmonic_centrality = .integration_processed_raw_method(network_payload, "Harmonic_centrality", ties_method, decreasing = TRUE),
    Degree_centrality = .integration_processed_raw_method(network_payload, "Degree_centrality", ties_method, decreasing = TRUE),
    P1 = .integration_processed_raw_method(network_payload, "P1", ties_method, decreasing = FALSE),
    P2 = .integration_processed_raw_method(network_payload, "P2", ties_method, decreasing = FALSE),
    P3 = .integration_processed_raw_method(network_payload, "P3", ties_method, decreasing = FALSE)
  )
  processed <- Filter(Negate(is.null), processed)

  centrality_tables <- Filter(
    function(x) !is.null(x) && nrow(x),
    lapply(c("TrustRank", "Harmonic_centrality", "Degree_centrality"), function(method) {
      .integration_processed_rank_input(processed, method)
    })
  )
  centrality_harmonized <- if (length(centrality_tables)) {
    Reduce(function(x, y) dplyr::full_join(x, y, by = "Drug"), centrality_tables) %>%
      .integration_expand_network_drugs()
  } else {
    NULL
  }

  proximity_tables <- Filter(
    function(x) !is.null(x) && nrow(x),
    lapply(c("P1", "P2", "P3"), function(method) {
      table <- .integration_processed_rank_input(processed, method)
      if (is.null(table)) {
        return(NULL)
      }
      .integration_expand_network_drugs(table)
    })
  )
  proximity_input <- if (length(proximity_tables)) {
    Reduce(function(x, y) dplyr::full_join(x, y, by = "Drug"), proximity_tables) %>%
      dplyr::distinct(Drug, .keep_all = TRUE)
  } else {
    NULL
  }
  proximity_agg <- .integration_run_raw_network_aggregation(
    input_df = proximity_input,
    prefix = "Network_proximity",
    ties_method = ties_method,
    prior = prior,
    num_bin = num_bin
  )

  diffusion_input <- .integration_diffusion_table(network_payload)
  if (!is.null(diffusion_input)) {
    diffusion_input <- .integration_expand_network_drugs(diffusion_input)
  }
  diffusion_agg <- .integration_run_raw_network_aggregation(
    input_df = diffusion_input,
    prefix = "Diffusion",
    ties_method = ties_method,
    prior = prior,
    num_bin = num_bin
  ...
  network_tables <- Filter(
    function(x) !is.null(x) && is.data.frame(x) && nrow(x),
    list(
      centrality_harmonized,
      proximity_agg$harmonized,
      diffusion_agg$harmonized
    )
  )
  if (!length(network_tables)) {
    stop("No raw network method tables were available for integration.", call. = FALSE)
  }

  network_harmonized <- Reduce(function(x, y) dplyr::full_join(x, y, by = "Drug"), network_tables) %>%
    .integration_expand_network_drugs()

  list(
    Processed = processed,
    RankAggregation = c(
      list(Network_proximity_input = proximity_input, Diffusion_input = diffusion_input),
      proximity_agg$aggregations,
      diffusion_agg$aggregations,
      list(Network_Harmonized_Raw = network_harmonized)
    ),
    Network_Harmonized = network_harmonized
  )
}
